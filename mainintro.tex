...
% $Id: mainintro.tex,v 1.23 2009/08/25 20:05:10 ith Exp $
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: mainintro.tex,v 1.23 2009/08/25 20:05:10 ith Exp $}
...
\begin{eqnarray}
 ...
 g_{00} &=& -a^2 (1 + 2\phi) \,,\\
 g_{0i} &=& a^2 (B_{,i} - S_i) \,,\\
 g_{ij} &=& a^2\left[ (1 - 2\psi)\delta_{ij} + 2E_{,ij} + 2F_{(i,j)} + h_{ij}
\right] \,.
\end{eqnarray}
% 
The vectors $S_i$ and $F_i$ are divergence free and the tensor part $h_{ij}$
is divergence free and traceless:
% 
\begin{equation}
 S^k_{~,k} = 0\,, \quad F^k_{~,k}=0\,; \qquad h^{ik}_{~~,k} = 0\,,
  \quad h^k_{~k}= 0\,.
\end{equation}
% 
...
consider them here \cite{book:liddle}.
...
spatial hypersurface are related to those in another, i.e. a threading. We will
employ the uniform curvature gauge in which spatial hypersurfaces are flat. It
is also known as the flat gauge.

% % 
% % One way to alleviate the problem of gauge choice is to consider gauge
% invariant % variables which are the same in all gauges.  In order to construct
% gauge % invariant quantities, the gauge transformation must be considered in
% depth % which will shall do in Chapter~\ref{ch:perts}. 


The equations of motion of the first order scalar field perturbation $\dvp1$ is
% 
\begin{equation}
 \label{eq:dp1eom-intro}
\dvp1''+2\H\dvp1'+2a^2 \Uphi \phi
-\nabla^2\dvp1-\vp_{0}'\nabla^2 B
-\vp_{0}'\phi' + a^2 \Upp \dvp1
=0\,,
\end{equation}
