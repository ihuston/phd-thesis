...
% $Id: mainintro.tex,v 1.30 2009/08/28 16:30:16 ith Exp $
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: mainintro.tex,v 1.30 2009/08/28 16:30:16 ith Exp $}
...
$V(\varphi)$. Inflation requires $P<-E/3$ so from \eq{eq:EandP-intro} inflation
...
making the friction $H\dot{\vp}$ in \eq{eq:phieom1-intro} dominant. The
...
The transformation vector at first order  $\xi_1^\mu$ can be split into scalar
and vector parts
% 
\begin{equation}
...
% Quantum perts
% Redefine perts in terms of quantum modes. 
\addtodo{Finish quantum perturbation section}
...
\cite{Unruh1976a}. In selecting the vacuum we must choose one of the many
equivalent options.
To do this we consider the far past where $\eta\rightarrow -\inf$ and all modes
...
% 
In the limit $\eta\rightarrow -\inf, k/aH\rightarrow \inf$ the mode equation
...
apply to non slow-roll models so long as they are applied well inside the
horizon.
...
\end{equation}
% 
where $\langle \ldots \rangle$ denotes the ensemble average. The power spectrum
...
slowly rolling mostly require their more complicated mode equations to be
numerically solved. 

We have discussed in depth the scalar perturbations but there are also
tensor perturbations which can be produced. The tensor perturbation $h_{ij}$
...
\addtodo{Add discussion of cph vs cs}
...
the canonical one. Again letting $u=a\dvp1$, the Mukhanov equation for the
Fourier modes, \eq{eq:ueq-intro}, now contains contributions from $c_s$
\cite{gm}:
% 
\begin{equation}
...
inside and outside the ``sound horizon'' $kc_s = aH$. Far inside the sound
...
Following the same analysis above we find that the amplitudes of the curvature 
perturbations 
generated during inflation change in this case and is given by \cite{gm}
% 
\begin{equation} 
...
This should be evaluated after the modes have exited the normal horizon $k\ll
aH$. 
...
\end{equation}
% 
where translation invariance imposes the conservation of the $\kb_i$ and the bispectrum
depends only on the magnitude of each wavenumber. Any deviation from gaussianity will result in a
non-zero bispectrum value. Because of the delta-function, the wavevectors form triangles in Fourier
space and $B$ is only a function of two variables. The bispectrum generated by inflationary models
takes two main triangular shapes, squeezed and equilateral.
 
One wavevector is significantly shorter than the other two in the squeezed case, \ie $k_3 \ll
k_1,k_2$. 
% 
\begin{equation} 
...
\subsection{Non-gaussianity in non-canonical inflation}
 In the limit  
...
forefront of this increase. Ground and balloon based missions including CBI
...
Piacentini2006} have complemented these.
...
\cite{Freedman2001}. In combination with this and data from Baryon
...
\cite{Wood-Vasey2007}, the WMAP 5-Year data release (WMAP5) constrains the
universe to be within two percent of the flat $\Omega =1, K=0$ case outlined in
