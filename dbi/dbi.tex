% % % % % % % % % % % % % % % % % % % % % 
% dbi.tex - Ian Huston
% $Id: dbi.tex,v 1.4 2009/07/12 19:27:47 ith Exp $
% % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: dbi.tex,v 1.4 2009/07/12 19:27:47 ith Exp $}
% \input{paper1}
...
Units are chosen such that $\MP \equiv (8\pi G )^{-1/2}= 2 
.4 \times 10^{18}\, {\rm GeV}$ 
...
\begin{eqnarray}
\label{Friedmann}
3 \MP^2 H^2(\phi ) = V(\phi ) -T(\phi ) 
\left[ 1- \sqrt{1+4\MP^4T^{-1} H'^2} \right] \\
\label{useful}
\dot{\phi} = - \frac{2\MP^2H'}{\sqrt{1+4\MP^4 T^{-1} H'^2}} \, ,
\end{eqnarray}
...
= \frac{XP_{,X}}{\MP^2H^2} 
= \frac{2\MP^2}{\gamma} \left( \frac{H'}{H} \right)^2 \\
\label{defeta}
\eta \equiv  \frac{2\MP^2}{\gamma}\frac{H''}{H} \\
\label{defs}
s \equiv \frac{\dot{c_s}}{c_sH} 
= \frac{2\MP^2}{\gamma} \frac{H'}{H}\frac{\gamma'}{\gamma}  \,,
\end{eqnarray}
...
\begin{eqnarray}
\label{amplitudes}
P_S^2= \frac{1}{8 \pi^2 \MP^2} \frac{H^2}{c_s \epsilon}
= \frac{H^4}{4\pi^2\dot{\phi}^2}
\\
P_T^2 = \frac{2}{\pi^2} \frac{H^2}{\MP^2} 
\\
\label{indices}
...
$\MP^2=V_6 \kappa_{10}^{-2}$, where $\kappa_{10}^2 \equiv 
...
that $\dot{\phi}^2 /\MP^2 = 2\epsilon H^2/P_{,X}$.
This implies that the variation of the inflaton field 
is given by the Lyth bound 
\cite{lyth,bmpaper}
\begin{equation}
\label{rtheory}
\frac{1}{\MP^2} \left( \frac{d\phi}{d {\cal{N}}} \right)^2 =
\frac{r}{8} \,,
\end{equation}
...
\left( \frac{\Delta \phi}{\MP} \right)^2_{*} < 
...
\begin{equation}
\label{boundpower6}
\left( \frac{\Delta \phi}{\MP} \right)^6_{*} 
< \frac{\pi^3}{16\mathrm{Vol}(X_5)} r^2 P_S^2 
\left( 1+ \frac{1}{3f_{NL}} \right)  \,.
\end{equation}
Hence, employing the Lyth bound in the form $(\Delta \phi_* / \MP )^2 \simeq 
r (\Delta {\cal{N}}_*)^2 /8$  
...
\mp \sqrt{\frac{r}{8}} \left( \frac{T'}{T} \MP \right)_*  \,.
\end{equation}
...
brane was near the tip of the throat and $\phi \ll \MP$
...
\begin{equation}
\label{observef1}
\left( \frac{f_1}{\MP^4} \right)_{*} \simeq \frac{\pi^2}{16} r^2P_S^2
\left( 1+ \frac{1}{3f_{NL}} \right)  \,.
\end{equation}
...
\left( \frac{\Delta \phi}{\MP} \right)^2_{*} \simeq 
...
\begin{equation}
\label{f2bound}
\frac{f_2 (\phi_*)\MP^4}{N} > \frac{(\Delta {\cal{N}}_*)^2}{\pi^2} 
\frac{1}{rP_S^2} \,.
\end{equation}
...
\begin{equation}
\label{f2IRlolwer}
\frac{f_2\MP^4}{N} > \frac{\sqrt{3} (\Delta {\cal{N}}_*)^2}{4\pi^2}
...
\left( \frac{\MP}{h_{\rm cut}m_s} \right)^4  \,.
\end{equation}
Assuming the typical values $m_s \sim 10^{-2}\MP$, 
...
\begin{equation}
\label{loglimit}
\frac{8\pi K}{3Mg_s} -\ln N > 4 \ln \left( \frac{m_s}{g_s^{1/4}\MP} \right)
-\ln \left( 
...
(1-n_s)P_S^2} \frac{1}{g_s} \left( \frac{m_s}{\MP} \right)^4 \,.
\end{equation}
