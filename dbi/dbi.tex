% % % % % % % % % % % % % % % % % % % % % 
% dbi.tex - Ian Huston
% $Id: dbi.tex,v 1.32 2009/09/18 17:07:09 ith Exp $
% % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: dbi.tex,v 1.32 2009/09/18 17:07:09 ith Exp $}
...
In this chapter two bounds will be derived which constrain the standard DBI
...
In Chapter~\ref{ch:multibrane} we show that such conditions can be realized
in a recently proposed IR version of DBI inflation driven
...
\end{equation}
% 
where the effective number of e-foldings $\Neff$ defined in
\eq{eq:Neff-dbiintro} 
...
inflation then $\Neff = \N_\mathrm{end}$, the total number of e-foldings of
inflation.
...
\nnl \equiv \frac{\d \ln |\fnleq|}{\d\ln k}\,. 
\end{equation}
% 
...
$\fnleqeq > -151$ \cite{Komatsu:2008hk} that the gravitational wave amplitude 
...
\begin{equation}
\label{eq:nnlbound}
\nnl \simeq -2s > -3 (1-n_s ) \fnleq > -0.078 \fnleq \,.
\end{equation}
% 
...
Consequently, 
by saturating the WMAP5 limit $\fnleqeq > -151$, we arrive at 
...
1-n_s = \frac{r}{4} \sqrt{1-3\fnleq}
 +\frac{\nnl}{1-3\fnleq} \mp \sqrt{\frac{r}{4f_1f_2}} \left( 
...
\eq{eq:genphivary1} that\footnote{We are 
...
$\nnl/\fnleq$ yields an {\em upper} limit on the tensor-scalar ratio:
% 
\begin{equation}
\label{eq:IRupper}
