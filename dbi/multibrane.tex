...
% $Id: multibrane.tex,v 1.43 2009/09/28 14:06:43 ith Exp $
% % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: multibrane.tex,v 1.43 2009/09/28 14:06:43 ith Exp $}
...
towards the tip of the throat in the UV scenario a complementary lower bound
on $r$ can be derived such that the two bounds are incompatible for the
current values of the observational parameters. In this chapter we will explore
...
into the BM bound (\ref{eq:genBMbound}) and the bound (\ref{eq:LHbound})
implies that
%  
\begin{equation}
...
model in two limit, arbitrarily large and small finite $n$. The large $n$ model
is similar to the single brane case. Using the results of
Section~\ref{sec:relaxing-dbi} we showed that the large $n$ case is strongly
...
tensor-scalar ratio. In Chapter~\ref{ch:multi} we described a class of actions
