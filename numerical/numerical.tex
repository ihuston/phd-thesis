...
% $Id: numerical.tex,v 1.37 2009/11/05 15:32:16 ith Exp $
% % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: numerical.tex,v 1.37 2009/11/05 15:32:16 ith Exp $}
...
perturbations in \eq{eq:fontime} is integrated using a standard
...
\addtodo{Change to incorporate new results}
It should be noted that these tests only show the relative errors in the
computation of the convolution term $\A$, corresponding to the most straightforward
term in \eq{eq:AtoD-num}. They
do not represent
...
analytic result. \eq{eq:err-analytic-num} gives some indication of
...
I_\B = 2\pi \int_\kmin^\kmax q^2 \dvp1(q) \B(k,q) \d q \,,
\end{equation}
% 
...
% 
 &= -2\pi\alpha^2 \int_{\kmin}^{\kmax} \d q\, q^{\frac{3}{2}} \int_0^\pi 
     \left( \left(k^2 + q^2 -2kq \cos\theta\right)^{-\frac{1}{4}} 
            +\frac{i}{\beta}\left(k^2 + q^2 -2kq \cos\theta\right)^{\frac{1}{4}}
