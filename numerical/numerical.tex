...
% $Id: numerical.tex,v 1.29 2009/10/12 10:23:55 ith Exp $
% % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: numerical.tex,v 1.29 2009/10/12 10:23:55 ith Exp $}
...
system closely follows the work done at first order by Martin
...
equations in Chapter~\ref{ch:perts} is the requirement to specify a finite
...
the Bunch-Davies vacuum initial conditions in Section
\ref{sec:perts-intro} blow up. The standard workaround is to implement a cutoff
...
We will consider three different ranges of $k$ modes in producing the results in
...
number of modes at second order is one greater than a power of two. This enables
...
background field and first and second order field perturbations. These form the
...
The convolution terms which are present in the Fourier space equations are
considered in spherical polar coordinates and split into calculable units. 

To test the numerical procedure four potentials have been used. These are defined in
...
indentically zero at the beginning of the simulation as explained in
Section~\ref{sec:initconds-num}. The section also describes the method and timing of
...
where this is possible. The choice of parameters in the calculation is informed by
...
The Klein-Gordon equations in Chapter~\ref{ch:perts} are not set up for a
...
number of e-foldings \eqref{eq:nefolddefn-intro}, and we use 
%
\begin{equation}
\label{eq:def-ntime}
...
\end{equation}
%
as our time variable instead of conformal time, measured from
...
If $a$ is set to be $1$ today we can calculate
$a_{\mathrm{init}}$ once the background run is complete and the number of e-foldings
of inflation is determined.
The value of $a$ at the end of inflation, $a_\mathrm{end}$, is calculated using the
connection equation (see for example
Eq.~(3.19) in \Rref{book:liddle} or Eq.~(7) in \Rref{Peiris:2008be}). 
If $a_0$ is the value of $a$ today we can relate these quantities by
% 
\begin{equation}
...
density and $a$ in matter and radiation dominated eras and the Friedmann
equation \eqref{eq:Friedmann1-intro} from Section~\ref{sec:frw-intro} we can write
% 
\begin{equation}
 \label{eq:conn2-num}
...
at the end of inflation, end of reheating and matter-radiation equality. The value
...
The mean value for $\Omega_m h^2$ given by WMAP5 + BAO + SN measurements is
...
\end{equation}
% 
For the results in Chapter~\ref{ch:results} it has been assumed that
reheating occurs instantaneously at the end of
inflation and $H_\mathrm{reh} = H_\mathrm{end}$. This gives approximately 65
e-foldings from the end of inflation until now. It also fixes the horizon crossing
time of the WMAP pivot scale $\kwmap=0.002\Mpc^{-1}$ to be about 60 e-foldings before
the end of inflation. Because $H$ is not kept fixed during the calculation the
number of e-foldings between horizon crossing and the end of inflation will differ
depending on the potential of the model and the evolution of $H$. 
...
potential $V$ and indeed the numerical code can use any reasonable
...
but a modular system allows other potentials to be used instead. 
...
potentials have been chosen. They are not meant to be an exhaustive selection
but do exhibit an interesting variety of behaviours. The potentials used are
% 
\begin{enumerate}
...
be ended by the decay of another field which will not be simulated. Instead
inflation will be manually set to end when $\vp \simeq 8$. By taking a value of
$V_0 = 5\e{-10}\Mpl^{4}$ a blue spectrum ($n_s>1$) can be obtained
...
give the background evolution described below and shown in Figure~\ref{fig:eps}. As
the evolution will quickly reach the attractor solution the choice for $\dN{\vp_0}$
...
-(aH(1-\bar{\varepsilon}^2_H))^{-1}$, when $\bar{\varepsilon}_H$ changes slowly. For
...
rewritten as two first order (in $\N$ derivatives) equations, is
...
$\bar{\varepsilon}^2_H = \varepsilon_H= -\dN{H}/H$ first becomes greater than or
equal to $1$
...
conditions for the first order system are then set as outlined above.
%
\begin{figure}
\centering
...
perturbations in \eq{eq:fontime} is calculated using a standard
...
1.2\e{-4}\Mpc^{-1}\,,
\nonumber\\ 
K_3 &= \left[0.52\e{-5}, 0.39\right]\Mpc^{-1}\,, &\Delta k = 3.8\e{-4}\Mpc^{-1}
\,.
\end{align}
% 
...
% 
\begin{equation}
 I_\A (k) = 2\pi \int_{\kmin}^{\kmax} q^2 \dvp1(q) \A(k, q) \d q \,,
\end{equation}
% 
...
\label{eq:err-analytic-num}
 I_\A(k) = \frac{\pi}{18}\frac{\alpha^2}{k} \Bigg\{ 
&3k^3\Bigg[\log\left(2\sqrt{k}\right) - \frac{\pi}{2}  + 
\arctan\left(\sqrt{\frac{\kmin}{k-\kmin}}\right) \nonumber \\
% 
 &+ \log\left(2\left(\sqrt{\kmin} + \sqrt{\kmin + k}\right)\right)
- \log\left(2\left(\sqrt{\kmax} + \sqrt{\kmax + k}\right)\right) \nonumber \\
% 
&- \log\left(2\left(\sqrt{\kmax} + \sqrt{\kmax - k}\right)\right) \Bigg]  \nonumber\\
% 
&+ \sqrt{\kmax}\bigg[\sqrt{\kmax -k} \left(-3k^2 + 14k\kmax - 8\kmax^2\right)
\nonumber \\
%  
 &\qquad+ \sqrt{\kmax +k} \left(3k^2 + 14k\kmax + 8\kmax^2\right)\bigg] \nonumber\\ 
% 
&- \sqrt{\kmin}\bigg[\sqrt{k -\kmin} \left(3k^2 - 14k\kmax +
8\kmax^2\right) \nonumber \\
% 
&\qquad- \sqrt{k +\kmin} \left(3k^2 + 14k\kmax + 8\kmax^2\right)\bigg] \Bigg\} \,.
\end{align}
%
...
computation of the $\A$ convolution term, the most straightforward term in
\eq{eq:AtoD-num}, only and
do not
represent
...
% =========================================================== %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
This chapter described the implementation of the numerical calculation of second
...
integrals in \eq{eq:KG2-fourier-sr-num} can be considered in spherical polar
coordinates and split into four sub-terms $\A$--$\wt{\D}$. Computing the source term
\eq{eq:KG2-src-ntime} which is written using $\A$--$\wt{\D}$ is the most complex and
...
potentials was determined by comparing the calculated $\Pr$ with the WMAP5 value.
...
second order perturbations are set to zero to begin. This choice concentrates focus
on the generation of second order effects only during the observable period of
inflation.


The technical implementation of the code was discussed in
Section~\ref{sec:impl-num}. There are four stage in the procedure. First the
...
used in the third stage to calculate the source term \eq{eq:KG`2-src-ntime} at each
time for each $k$. Finally the second order perturbations equations are solved using
...
$N_\theta$ and $N_k$ were set by minimising the relative error between these results.

In Chapter~\ref{ch:perts} the theoretical equations for second order perturbations
...
improved procedure will be discussed.