...
% $Id: numerical.tex,v 1.34 2009/10/14 17:00:27 ith Exp $
% % % % % % % % % % % % % % % % % % % % % 
% Redefine CVSRevision for this section
\renewcommand{\CVSrevision}{\version$Id: numerical.tex,v 1.34 2009/10/14 17:00:27 ith Exp $}
...
single field potential provided that it gives a period of inflationary expansion in
...
be ended by the decay of another field but this will not be implemented. Instead
inflation will be manually set to end when $\vp \simeq 8$. By taking a value of
$U_0 = 5\e{-10}\Mpl^{4}$ a blue spectrum ($n_s>1$) can be obtained
...
during this range. For example the initial value $\vp_0 = 18\Mpl$ for the
$\msqphisq$ model 
...
intensive and can be run in parallel, as the calculation at each time step is
...
particular it is important to know whether the values
picked for $N_\theta$, the number of discretised $\theta$s, $\Delta k$, the size of
the
...
% 
\begin{equation}
 I_\A (k) = \int_{\kmin}^{\kmax} q^2 \dvp1(q) \A(k, q) \d q \,,
\end{equation}
% 
then putting in $\dvp1(k) = \alpha/\sqrt{k}$ gives
% 
\begin{equation}
 I_\A(k) = \alpha^2 \int_{\kmin}^{\kmax} \d q\, q^{\frac{3}{2}}
...
\label{eq:err-analytic-num}
 I_\A(k) = \frac{1}{36}\frac{\alpha^2}{k} \Bigg\{ 
...
terms, although this is a goal for future work.
